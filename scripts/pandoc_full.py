...
def main(
    ...
    if media_src.exists():
        dest_dir = Path('images/posts') / post_id
        if dest_dir.exists():
            print(f'destination {dest_dir} already exists, skipping media move')
        else:
            print(f'moving media from {media_src} to {dest_dir}')
            dest_dir.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(media_src), str(dest_dir))
    else:
        print(f'no media directory found at {media_src}, skipping media move')
