...
def load_and_process(document_path, author, tags):
    ...
    # Ensure we have a title/subtitle even if the source markdown
    # doesn't include YAML frontmatter.
    if not post.get('title'):
        ...
        if second_idx is not None:
            subtitle = lines[second_idx].strip()

        post['title'] = clean_title(title)
        if not post.get('subtitle'):
            post['subtitle'] = clean_title(subtitle) if subtitle else ''

        # Remove title/subtitle lines from the rendered content so they
        # don't appear twice in the final article body.
        new_lines = []
        for idx, raw in enumerate(lines):
    ...
    post_path = Path(f'content/blog/{post_id}.md')
